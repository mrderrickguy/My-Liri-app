...
function retrieveTweets() {
    ...
    var keys = new Twitter('./Keys.js');

    // Set the 'screen_name' to my Twitter handle
    var params = {screen_name: 'Mr_DGuy', count: 20};
...
function spotifySong(song) {
    // Append the command to the log file
    fs.appendFile('./random.txt', 'User Command: node liri.js spotify-this-song ' + song + '\n\n', (err) => {
        if (err) throw err;
    });
    ...
    spotify.search({ type: 'track', query: search}, function(error, data) {
        if (error) {
            ...
            // Append the error string to the log file
            fs.appendFile('./random.txt', errorStr1, (err) => {
                if (err) throw err;
            ...
            if (!songInfo) {
                ...
                // Append the error string to the log file
                fs.appendFile('./random.txt', errorStr2, (err) => {
                    if (err) throw err;
                ...
                // Append the output to the log file
                fs.appendFile('./random.txt', 'LIRI Response:\n\n' + outputStr + '\n', (err) => {
                    if (err) throw err;
                    console.log(outputStr);
                });
            }
...
function retrieveOBDBInfo(movie) {
    // Append the command to the log file
    fs.appendFile('./random.txt', 'User Command: node liri.js movie-this ' + movie + '\n\n', (err) => {
        if (err) throw err;
    });
    ...
    var queryStr = 'http://www.omdbapi.com/?t=' + search + '&plot=full&tomatoes=&api=e48ba4b3';
...
function doAsYerTold() {
    // Append the command to the log file
    fs.appendFile('./random.txt', 'User Command: node liri.js spotify-this-song \n\n', (err) => {
        if (err) throw err;
    });
